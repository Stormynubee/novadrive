...
import { queryRouteSafety, type SafetyAnalysis } from '../lib/routing/safetyEngine';
...
export function PlanCorridorScreen() {
  ...
  useEffect(() => {
    let active = true;
    void (async () => {
      const res = await queryRouteSafety(osrmPolyline, destination);
      if (active) {
        setSafetyAnalysis(res);
      }
    })();
    return () => {
      active = false;
                ...
                <View style={styles.safetyAnalysisCard}>
                  <View style={styles.safetyAnalysisHeader}>
                    ...
                    <View style={styles.sourceBadge}>
                      <HudText variant="mono" style={styles.sourceBadgeText}>
                        {safetyAnalysis.dataSource === 'cloud'
                          ? '● LIVE CLOUD'
                          : safetyAnalysis.dataSource === 'sqlite'
                          ? '● OFFLINE DB'
                          : '● FAILSAFE PRESETS'}
                      </HudText>
                    </View>
